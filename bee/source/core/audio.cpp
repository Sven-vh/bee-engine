...
void Audio::LoadBank(const fs::path& filename)
{
    // check if the bank already exists
    int hash = static_cast<int>(std::hash<std::string>{}(filename.string()));
    if (m_banks.find(hash) != m_banks.end()) return;

    // try to load the bank
    FMOD::Studio::Bank* bank = nullptr;
    const auto& real_filename = Engine.FileIO().GetPath(FileIO::Directory::Assets, filename);
    auto result = m_system->loadBankFile(real_filename.string().c_str(), FMOD_STUDIO_LOAD_BANK_NORMAL, &bank);
    if (result != FMOD_OK)
    {
        Log::Error("FMOD bank with filename {} could not be loaded!", filename.string());
        return;
    }
...
void Audio::UnloadBank(const fs::path& filename)
{
    int hash = static_cast<int>(std::hash<std::string>{}(filename.string()));
    if (m_banks.find(hash) == m_banks.end())
    {
        Log::Error("FMOD bank with filename {} could not be unloaded!", filename.string());
        return;
    }
...
int Audio::StartEvent(const std::string& name)
{
    ...
    if (result != FMOD_OK)
    {
        Log::Error("FMOD event with m_name {} does not exist!", name);
        return -1;
    }
    ...
    if (result != FMOD_OK)
    {
        Log::Error("FMOD event instance with m_name {} could not be created!", name);
        return -1;
    }
...
void Audio::LoadSound(const fs::path& filename, bool isMusic)
{
    ...
    const auto& real_filename = Engine.FileIO().GetPath(FileIO::Directory::Assets, filename);
    FMOD_RESULT result = m_core_system->createSound(real_filename.string().c_str(), mode, nullptr, &sound);
    if (result != FMOD_OK)
    {
        Log::Error("Sound with filename {} could not be loaded!", filename.string());
        return;
    }
...
int Audio::PlaySound(const fs::path& filename)
{
    ...
    {
        Log::Error("Sound with filename {} has not been loaded!", filename.string());
        return -1;
    }
