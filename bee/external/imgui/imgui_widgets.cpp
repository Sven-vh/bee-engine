...
bool ImGui::Button(const char* label, const ImVec2& size_arg) { return ButtonEx(label, size_arg, ImGuiButtonFlags_None); }
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // close a window. (this isn't the regular behavior of buttons, but it doesn't affect the user much because navigation tends
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    ImVec2 center = bb.GetCenter();
    if (hovered || held) window->DrawList->AddCircleFilled(center + ImVec2(0, -0.5f), g.FontSize * 0.5f + 1.0f, bg_col, 12);
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    const float scrollbar_size =
        window
            ->ScrollbarSizes[axis ^ 1];  // (ScrollbarSizes.x = width of Y scrollbar; ScrollbarSizes.y = height of X scrollbar)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (mixed_value)
    {
        ...
        // This may seem awkwardly designed because the aim is to make ImGuiItemFlags_MixedValue supported by all widgets (not
        // just checkbox)
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // FIXME-WORKRECT: In theory we should simply be using WorkRect.Min.x/Max.x everywhere but it isn't aesthetically what
        // we want, need to introduce a variant of WorkRect for that purpose. (#4787)
...
void ImGui::Separator()
{
    ...
    flags |= ImGuiSeparatorFlags_SpanAllColumns;  // NB: this only applies to legacy Columns() api as they relied on Separator()
                                                  // a lot.
    SeparatorEx(flags);
}
...
bool ImGui::BeginComboPreview()
{
    ...
    if (window->SkipItems ||
        !window->ClipRect.Overlaps(g.LastItemData.Rect))  // FIXME: Because we don't have a ImGuiItemStatusFlags_Visible flag to
                                                          // test last ItemAdd() result
        return false;
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    if (format == nullptr) format = type_info->ScanFmt;
        ...
        data_type == ImGuiDataType_U64 || data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
    {
        ...
        // them into %f and %lf
        if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) format = type_info->ScanFmt;
                          ...
                          ImGuiSliderFlags flags)
{
    ...
    if (!(flags & ImGuiSliderFlags_NoRoundToFormat)) v_cur = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(format, data_type, v_cur);
                                 ...
                                 float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        // We special-case the extents because otherwise our fudging can lead to "mathematically correct" but non-intuitive
        // behaviors like a fully-left slider not actually reaching the minimum value
        if (t <= 0.0f)
            result = v_min;
        else if (t >= 1.0f)
            result = v_max;
                            ...
                            ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            const ImVec2 input_delta2 = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad,
                                                            ImGuiInputReadMode_RepeatFast,
                                                            0.0f,
                                                            0.0f);
...
bool ImGui::SliderAngle(const char* label,
                        ...
                        ImGuiSliderFlags flags)
{
    if (format == nullptr) format = "%.0f deg";
                            ...
                            const void* p_clamp_max)
{
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        DataTypeApplyFromText(data_buf, data_type, p_data, nullptr);
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    if (format == nullptr) format = DataTypeGetInfo(data_type)->PrintFmt;
    ...
    flags |= ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselves by comparing the actual data rather than
                                                // the string.
                        ...
                        void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        // read/debug.
        // FIXME-NAV: Pressing NavActivate will trigger general child activation right before triggering our own below. Harmless
        // but bizarre.
    ...
    const bool user_scroll_finish = is_multiline && state != nullptr && g.ActiveId == 0 &&
                                    g.ActiveIdPreviousFrame == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    const bool user_scroll_active =
        is_multiline && state != nullptr && g.ActiveId == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    ...
    if (g.ActiveId == id && state == nullptr) ClearActiveID();
...
{
    // This check is optional. Suppose we have two color widgets side by side, both widgets display different colors, but both
    // colors have hue and/or saturation undefined. With color check: hue/saturation is preserved in one widget. Editing color
    // in one widget would reset hue/saturation in another one. Without color check: common hue/saturation would be displayed in
    // all widgets that have hue/saturation undefined. g.ColorEditLastColor is stored as ImU32 RGB value: this essentially gives
    // us color equality check with reduced precision. Tiny external color changes would not be detected and this check would
    // still pass. This is OK, since we only restore hue/saturation _only_ if they are undefined, therefore this change flipping
    // hue/saturation from undefined to a very tiny value would still be represented in color picker.
    ImGuiContext& g = *GImGui;
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList,
                        ...
                        is_open ? ImGuiDir_Down : ImGuiDir_Right,
                        0.70f);
        if (g.LogEnabled) LogSetNextTextDecoration(">", nullptr);
...
// p_visible != nullptr && *p_visible == true  : show a small close button on the corner of the header, clicking the button will
// set *p_visible = false p_visible != nullptr && *p_visible == false : do not show the header at all Do not mistake this with the
...
// Tip: If your vertical size is calculated from an item count (e.g. 10 * item_height) consider adding a fractional part to
// facilitate seeing scrolling boundaries (e.g. 10.25 * item_height).
...
static bool IsRootOfOpenMenuSet()
{
    ...
    // Initially we used 'OpenParentId' to differentiate multiple menu sets from each others (e.g. inside menu bar vs loose menu
    // items) based on parent ID. This would however prevent the use of e.g. PuhsID() user code submitting menus. Previously
    // this worked between popup and a first child menu because the first child menu always had the _ChildWindow flag, making
    // hovering on parent popup possible while first child menu was focused - but this was generally a bug with other side
    // effects. Instead we don't treat Popup specifically (in order to consistently support menu features in them), maybe the
    // first child menu of a Popup doesn't have the _ChildWindow flag, and we rely on this IsRootOfOpenMenuSet() check to allow
    // hovering between root window/popup and first chilld menu.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // not allow hovering on parent menu) The first menu in a hierarchy isn't so hovering doesn't get accross (otherwise e.g.
        ...
        ImGuiLayoutType_Vertical)  // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != nullptr && !(window->Flags & ImGuiWindowFlags_MenuBar))
        {
            ...
            tb.y = ta.y + ImMax((tb.y - extra) - ta.y,
                                -ref_unit * 8.0f);  // triangle is maximum 200 high to limit the slope and the bias toward large
                                                    // sub-menus // FIXME: Multiply by fb_scale?
            tc.y = ta.y + ImMin((tc.y + extra) - ta.y, +ref_unit * 8.0f);
            moving_toward_other_child_menu = ImTriangleContainsPoint(ta, tb, tc, g.IO.MousePos);
            // GetForegroundDrawList()->AddTriangleFilled(ta, tb, tc, moving_toward_other_child_menu ? IM_COL32(0,128,0,128) :
    ...
    if (menu_is_open)
    {
        SetNextWindowPos(popup_pos, ImGuiCond_Always);  // Note: this is super misleading! The value will serve as reference for
...
bool ImGui::BeginMenu(const char* label, bool enabled) { return BeginMenuEx(label, nullptr, enabled); }
...
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(TabBarFindTabByID(tab_bar, window->TabId) == nullptr);
    ...
    if (!window->HasCloseButton)
        tab_flags |= ImGuiTabItemFlags_NoCloseButton;  // Set _NoCloseButton immediately because it will be used for first-frame
                                                       // width calculation.
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    if (tab1 == nullptr || (tab1->Flags & ImGuiTabItemFlags_NoReorder)) return false;
                      ...
                      ImGuiWindow* docked_window)
{
    ...
    const ImGuiID close_button_id = p_open ? GetIDWithSeed("#CLOSE", nullptr, docked_window ? docked_window->ID : id) : 0;
    bool just_closed;
    bool text_clipped;
