...
typedef FMOD_RESULT(F_CALL *FMOD_DSP_READ_CALLBACK)(FMOD_DSP_STATE *dsp_state,
                                                    float *inbuffer,
                                                    float *outbuffer,
                                                    unsigned int length,
                                                    int inchannels,
                                                    int *outchannels);
typedef FMOD_RESULT(F_CALL *FMOD_DSP_PROCESS_CALLBACK)(FMOD_DSP_STATE *dsp_state,
                                                       unsigned int length,
...
typedef FMOD_RESULT(F_CALL *FMOD_DSP_SHOULDIPROCESS_CALLBACK)(FMOD_DSP_STATE *dsp_state,
                                                              FMOD_BOOL inputsidle,
                                                              unsigned int length,
...
typedef FMOD_RESULT(F_CALL *FMOD_DSP_SETPARAM_DATA_CALLBACK)(FMOD_DSP_STATE *dsp_state,
                                                             int index,
                                                             void *data,
                                                             unsigned int length);
typedef FMOD_RESULT(F_CALL *FMOD_DSP_GETPARAM_FLOAT_CALLBACK)(FMOD_DSP_STATE *dsp_state,
                                                              int index,
                                                              float *value,
                                                              char *valuestr);
typedef FMOD_RESULT(F_CALL *FMOD_DSP_GETPARAM_INT_CALLBACK)(FMOD_DSP_STATE *dsp_state, int index, int *value, char *valuestr);
typedef FMOD_RESULT(F_CALL *FMOD_DSP_GETPARAM_BOOL_CALLBACK)(FMOD_DSP_STATE *dsp_state,
                                                             int index,
                                                             FMOD_BOOL *value,
                                                             char *valuestr);
typedef FMOD_RESULT(F_CALL *FMOD_DSP_GETPARAM_DATA_CALLBACK)(FMOD_DSP_STATE *dsp_state,
                                                             int index,
                                                             void **data,
                                                             unsigned int *length,
                                                             char *valuestr);
...
typedef void(
    F_CALL *FMOD_DSP_LOG_FUNC)(FMOD_DEBUG_FLAGS level, const char *file, int line, const char *function, const char *str, ...);
...
typedef FMOD_RESULT(F_CALL *FMOD_DSP_DFT_FFTREAL_FUNC)(FMOD_DSP_STATE *dsp_state,
                                                       int size,
                                                       const float *signal,
                                                       FMOD_COMPLEX *dft,
                                                       const float *window,
                                                       int signalhop);
typedef FMOD_RESULT(F_CALL *FMOD_DSP_DFT_IFFTREAL_FUNC)(FMOD_DSP_STATE *dsp_state,
                                                        int size,
...
#define FMOD_DSP_INIT_PARAMDESC_FLOAT_WITH_MAPPING(_paramstruct,                                         \
                                                   _name,                                                \
                                                   _label,                                               \
                                                   _description,                                         \
                                                   _defaultval,                                          \
...
#define FMOD_DSP_INIT_PARAMDESC_INT(_paramstruct,      \
                                    _name,             \
                                    _label,            \
                                    _description,      \
                                    _min,              \
...
#define FMOD_DSP_PAN_SUMSTEREOMATRIX(_state, _sourcespeakermode, _pan, _lowfrequencygain, _overallgain, _matrixhop, _matrix) \
    (_state)->functions->pan->sumstereomatrix(_state,                                                                        \
                                              _sourcespeakermode,                                                            \
                                              _pan,                                                                          \
                                              _lowfrequencygain,                                                             \
                                              _overallgain,                                                                  \
                                              _matrixhop,                                                                    \
                                              _matrix)
#define FMOD_DSP_PAN_SUMSURROUNDMATRIX(_state,                      \
                                       _sourcespeakermode,          \
                                       _targetspeakermode,          \
                                       _direction,                  \
                                       _extent,                     \
                                       ...
                                       _matrix,                     \
                                       _flags)                      \
    (_state)->functions->pan->sumsurroundmatrix(_state,             \
                                                _sourcespeakermode, \
                                                _targetspeakermode, \
                                                _direction,         \
                                                _extent,            \
                                                ...
                                                _matrix,            \
                                                _flags)
#define FMOD_DSP_PAN_SUMMONOTOSURROUNDMATRIX(_state,                      \
                                             _targetspeakermode,          \
                                             _direction,                  \
                                             _extent,                     \
                                             _lowfrequencygain,           \
                                             ...
                                             _matrixhop,                  \
                                             _matrix)                     \
    (_state)->functions->pan->summonotosurroundmatrix(_state,             \
                                                      _targetspeakermode, \
                                                      _direction,         \
                                                      _extent,            \
                                                      _lowfrequencygain,  \
                                                      ...
                                                      _matrixhop,         \
                                                      _matrix)
#define FMOD_DSP_PAN_SUMSTEREOTOSURROUNDMATRIX(_state,                      \
                                               _targetspeakermode,          \
                                               _direction,                  \
                                               _extent,                     \
                                               _rotation,                   \
                                               ...
                                               matrixhop,                   \
                                               _matrix)                     \
    (_state)->functions->pan->sumstereotosurroundmatrix(_state,             \
                                                        _targetspeakermode, \
                                                        _direction,         \
                                                        _extent,            \
                                                        _rotation,          \
