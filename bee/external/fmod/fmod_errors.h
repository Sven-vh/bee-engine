...
static const char *FMOD_ErrorString(FMOD_RESULT errcode)
{
    switch (errcode)
    {
        ...
        case FMOD_ERR_DSP_CONNECTION:
            return "DSP connection error.  Connection possibly caused a cyclic dependency or connected dsps with incompatible "
                   "buffer counts.";
        case FMOD_ERR_DSP_DONTPROCESS:
            return "DSP return code from a DSP process query callback.  Tells mixer not to call the process callback and "
                   "therefore not consume CPU.  Use this to optimize the DSP graph.";
        case FMOD_ERR_DSP_FORMAT:
            return "DSP Format error.  A DSP unit may have attempted to connect to this network with the wrong format, or a "
                   "matrix may have been set with the wrong size if the m_target unit has a specified channel map.";
        ...
        case FMOD_ERR_DSP_SILENCE:
            return "DSP return code from a DSP process query callback.  Tells mixer silence would be produced from read, so go "
                   "idle and not consume CPU.  Use this to optimize the DSP graph.";
        ...
        case FMOD_ERR_HEADER_MISMATCH:
            return "There is a version mismatch between the FMOD header and either the FMOD Studio library or the FMOD Low "
                   "Level library.";
        case FMOD_ERR_HTTP:
        ...
        case FMOD_ERR_INVALID_POSITION:
            return "An invalid seek m_position was passed to this function.";
        ...
        case FMOD_ERR_OUTPUT_ALLOCATED:
            return "Error initializing output m_device, but more specifically, the output m_device is already in use and cannot be "
                   "reused.";
        case FMOD_ERR_OUTPUT_CREATEBUFFER:
            return "Error creating hardware sound buffer.";
        case FMOD_ERR_OUTPUT_DRIVERCALL:
            return "A call to a standard soundcard driver failed, which could possibly mean a bug in the driver or resources "
                   "were missing or exhausted.";
        case FMOD_ERR_OUTPUT_FORMAT:
            return "Soundcard does not support the specified format.";
        case FMOD_ERR_OUTPUT_INIT:
            return "Error initializing output m_device.";
        case FMOD_ERR_OUTPUT_NODRIVERS:
            return "The output m_device has no drivers installed.  If pre-init, FMOD_OUTPUT_NOSOUND is selected as the output "
                   "mode.  If post-init, the function just fails.";
        ...
        case FMOD_ERR_RECORD:
            return "An error occurred trying to initialize the recording m_device.";
        ...
        case FMOD_ERR_SUBSOUNDS:
            return "The error occurred because the sound referenced contains subsounds when it shouldn't have, or it doesn't "
                   "contain subsounds when it should have.  The operation may also not be able to be performed on a parent "
                   "sound.";
        case FMOD_ERR_SUBSOUND_ALLOCATED:
            return "This subsound is already being used by another sound, you cannot have more than one parent to a sound.  "
                   "Null out the other parent's entry first.";
        case FMOD_ERR_SUBSOUND_CANTMOVE:
            return "Shared subsounds cannot be replaced or moved from their parent stream, such as when the parent stream is "
                   "an FSB file.";
