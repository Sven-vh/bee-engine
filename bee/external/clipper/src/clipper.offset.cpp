...
{
    return r.left > min_coord + abs_delta && r.right < max_coord - abs_delta && r.top > min_coord + abs_delta &&
...
inline bool AlmostZero(double value, double epsilon = 0.001) { return std::fabs(value) < epsilon; }
...
inline bool IsClosedPath(EndType et) { return et == EndType::Polygon || et == EndType::Joined; }
...
PointD IntersectPoint(const PointD& pt1a, const PointD& pt1b, const PointD& pt2a, const PointD& pt2b)
{
...
void ClipperOffset::DoSquare(Group& group, const Path64& path, size_t j, size_t k)
{
    ...
    if (j == k)
        ...
        vec = GetAvgUnitVector(PointD(-norms[k].y, norms[k].x), PointD(norms[j].y, -norms[j].x));
...
#ifdef USINGZ
    group.path.push_back(
        Point64(path[j].x + (norms[k].x + norms[j].x) * q, path[j].y + (norms[k].y + norms[j].y) * q, path[j].z));
#else
    group.path.push_back(Point64(path[j].x + (norms[k].x + norms[j].x) * q, path[j].y + (norms[k].y + norms[j].y) * q));
#endif
}
...
void ClipperOffset::DoRound(Group& group, const Path64& path, size_t j, size_t k, double angle)
{
    ...
    if (angle > -PI + 0.01)  // avoid 180deg concave
    {
        ...
        {
            offsetVec =
                PointD(offsetVec.x * step_cos_ - step_sin_ * offsetVec.y, offsetVec.x * step_sin_ + offsetVec.y * step_cos_);
#ifdef USINGZ
            group.path.push_back(Point64(pt.x + offsetVec.x, pt.y + offsetVec.y, pt.z));
#else
            group.path.push_back(Point64(pt.x + offsetVec.x, pt.y + offsetVec.y));
...
{
    for (Path64::size_type i = 0, j = path.size() - 1; i < path.size(); j = i, ++i) OffsetPoint(group, path, i, j);
...
void ClipperOffset::OffsetOpenPath(Group& group, Path64& path)
{
...
#ifdef USINGZ
            group.path.push_back(
                Point64(path[0].x - norms[0].x * group_delta_, path[0].y - norms[0].y * group_delta_, path[0].z));
#else
            group.path.push_back(Point64(path[0].x - norms[0].x * group_delta_, path[0].y - norms[0].y * group_delta_));
#endif
    ...
    for (Path64::size_type i = 1, k = 0; i < highI; ++i) OffsetPoint(group, path, i, k);

    // reverse normals
    for (size_t i = highI; i > 0; --i) norms[i] = PointD(-norms[i - 1].x, -norms[i - 1].y);
    norms[0] = norms[highI];
    ...
    switch (end_type_)
    {
        case EndType::Butt:
#ifdef USINGZ
            group.path.push_back(Point64(path[highI].x - norms[highI].x * group_delta_,
                                         path[highI].y - norms[highI].y * group_delta_,
                                         path[highI].z));
#else
            group.path.push_back(
                Point64(path[highI].x - norms[highI].x * group_delta_, path[highI].y - norms[highI].y * group_delta_));
#endif
    ...
    for (size_t i = highI, k = 0; i > 0; --i) OffsetPoint(group, path, i, k);
    group.paths_out.push_back(group.path);
}
...
void ClipperOffset::DoGroupOffset(Group& group)
{
    ...
    if (group.join_type == JoinType::Round || group.end_type == EndType::Round)
    {
        ...
        double arcTol = (arc_tolerance_ > floating_point_tolerance ? std::min(abs_group_delta_, arc_tolerance_)
                                                                   : std::log10(2 + abs_group_delta_) * default_arc_tolerance);
        double steps_per_360 = PI / std::acos(1 - arcTol / abs_group_delta_);
        if (steps_per_360 > abs_group_delta_ * PI) steps_per_360 = abs_group_delta_ * PI;  // ie avoids excessive precision
    ...
    for (path_iter = group.paths_in.begin(); path_iter != group.paths_in.end(); ++path_iter)
    {
        ...
        if (cnt == 0 || ((cnt < 3) && group.end_type == EndType::Polygon)) continue;
