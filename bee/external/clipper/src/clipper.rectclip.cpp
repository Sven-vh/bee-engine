...
inline bool GetIntersection(const Path64& rectPath, const Point64& p, const Point64& p2, Location& loc, Point64& ip)
{
    ...
    switch (loc)
    {
        case Location::Left:
            ...
            else if (p.y < rectPath[0].y && SegmentsIntersect(p, p2, rectPath[0], rectPath[1], true))
            {
        ...
        case Location::Top:
            if (SegmentsIntersect(p, p2, rectPath[0], rectPath[1], true))
                GetIntersectPoint(p, p2, rectPath[0], rectPath[1], ip);
            else if (p.x < rectPath[0].x && SegmentsIntersect(p, p2, rectPath[0], rectPath[3], true))
            {
                GetIntersectPoint(p, p2, rectPath[0], rectPath[3], ip);
                loc = Location::Left;
            }
            else if (p.x > rectPath[1].x && SegmentsIntersect(p, p2, rectPath[1], rectPath[2], true))
            {
        ...
        case Location::Bottom:
            if (SegmentsIntersect(p, p2, rectPath[2], rectPath[3], true))
                GetIntersectPoint(p, p2, rectPath[2], rectPath[3], ip);
            else if (p.x < rectPath[3].x && SegmentsIntersect(p, p2, rectPath[0], rectPath[3], true))
            {
                GetIntersectPoint(p, p2, rectPath[0], rectPath[3], ip);
                loc = Location::Left;
            }
            else if (p.x > rectPath[2].x && SegmentsIntersect(p, p2, rectPath[1], rectPath[2], true))
            {
                GetIntersectPoint(p, p2, rectPath[1], rectPath[2], ip);
                loc = Location::Right;
            }
...
inline bool AreOpposites(Location prev, Location curr) { return abs(static_cast<int>(prev) - static_cast<int>(curr)) == 2; }
...
void RectClip::GetNextLocation(const Path64& path, Location& loc, int& i, int highI)
{
    switch (loc)
    {
        case Location::Left:
...
void RectClip::ExecuteInternal(const Path64& path)
{
    ...
    while (i <= highI)
    {
        ...
        else if (prev != Location::Inside)
        {
            ...
            if (crossing_prev != Location::Inside) AddCorner(crossing_prev, loc);
        ...
        {
            loc = crossing_loc;
            if (first_cross_ == Location::Inside) first_cross_ = crossing_loc;
        }
    ...
    else if (loc != Location::Inside && (loc != first_cross_ || start_locs_.size() > 2))
    {
        ...
        if (loc != first_cross_) AddCorner(loc, HeadingClockwise(loc, first_cross_));
    }
}
...
Paths64 RectClip::Execute(const Paths64& paths, bool convex_only)
{
    ...
    for (const auto& path : paths)
    {
        ...
        {
            CheckEdges();
            for (int i = 0; i < 4; ++i) TidyEdges(i, edges_[i * 2], edges_[i * 2 + 1]);
        }
