...
inline bool IsOpenEnd(const Active& ae) { return IsOpenEnd(*ae.vertex_top); }
...
inline bool IsFront(const Active& e) { return (&e == e.outrec->front_edge); }

inline bool IsInvalidPath(OutPt* op) { return (!op || op->next == op); }
...
inline bool IsHorizontal(const Active& e) { return (e.top.y == e.bot.y); }

inline bool IsHeadingRightHorz(const Active& e) { return e.dx == -std::numeric_limits<double>::max(); }

inline bool IsHeadingLeftHorz(const Active& e) { return e.dx == std::numeric_limits<double>::max(); }
...
inline bool IsSamePolyType(const Active& e1, const Active& e2) { return e1.local_min->polytype == e2.local_min->polytype; }
...
inline bool IsMaxima(const Vertex& v) { return ((v.flags & VertexFlags::LocalMax) != VertexFlags::None); }

inline bool IsMaxima(const Active& e) { return IsMaxima(*e.vertex_top); }
...
double Area(OutPt* op)
{
    ...
    do
    {
        result += static_cast<double>(op2->prev->pt.y + op2->pt.y) * static_cast<double>(op2->prev->pt.x - op2->pt.x);
        op2 = op2->next;
    } while (op2 != op);
    return result * 0.5;
}
...
{
    return op.next->next == op.prev && (PtsReallyClose(op.prev->pt, op.next->pt) || PtsReallyClose(op.pt, op.next->pt) ||
...
inline bool OutrecIsAscending(const Active* hotEdge) { return (hotEdge == hotEdge->outrec->front_edge); }
...
inline bool IsJoined(const Active& e) { return e.join_with != JoinWith::None; }

inline void SetOwner(OutRec* outrec, OutRec* new_owner)
{
    // precondition1: new_owner is never null
    while (new_owner->owner && !new_owner->owner->pts) new_owner->owner = new_owner->owner->owner;
...
void ClipperBase::Reset()
{
    ...
    for (i = minima_list_.rbegin(); i != minima_list_.rend(); ++i) InsertScanline((*i)->vertex->pt.y);
...
void ClipperBase::AddPaths(const Paths64& paths, PathType polytype, bool is_open)
{
    ...
    for (const Path64& path : paths)
    {
        ...
        if (!is_open && prev_v->pt == v0->pt) prev_v = prev_v->prev;
        prev_v->next = v0;
        v0->prev = prev_v;
...
void ClipperBase::InsertScanline(int64_t y) { scanline_list_.push(y); }
...
bool ClipperBase::PopScanline(int64_t& y)
{
    ...
    scanline_list_.pop();
    while (!scanline_list_.empty() && y == scanline_list_.top()) scanline_list_.pop();  // Pop duplicates.
    return true;
}
    ...
    while (PopLocalMinima(bot_y, local_minima))
    {
        ...
        if (right_bound)
        {
            ...
            if (contributing)
            {
                ...
                if (!IsHorizontal(*left_bound)) CheckJoinLeft(*left_bound, left_bound->bot);
            }

            while (right_bound->next_in_ael && IsValidAelOrder(*right_bound->next_in_ael, *right_bound))
            {
...
OutPt* ClipperBase::AddLocalMinPoly(Active& e1, Active& e2, const Point64& pt, bool is_new)
{
    ...
    else
    {
        ...
        if (prevHotEdge)
        {
            if (using_polytree_) SetOwner(outrec, prevHotEdge->outrec);
...
OutPt* ClipperBase::AddOutPt(const Active& e, const Point64& pt)
{
    ...
    if (to_front)
    {
        if (pt == op_front->pt) return op_front;
    }
...
#ifdef USINGZ
    if (zCallback_) zCallback_(prevOp->pt, splitOp->pt, splitOp->next->pt, nextNextOp->pt, ip);
#endif
...
OutPt* ClipperBase::IntersectEdges(Active& e1, Active& e2, const Point64& pt)
{
    ...
    if (has_open_paths_ && (IsOpen(e1) || IsOpen(e2)))
    {
        ...
        switch (cliptype_)
        {
            case ClipType::Union:
                ...
                break;
            default:
                if (edge_c->local_min->polytype == PathType::Subject) return nullptr;
        }

        switch (fillrule_)
        {
            case FillRule::Positive:
                if (edge_c->wind_cnt != 1) return nullptr;
                break;
            case FillRule::Negative:
                if (edge_c->wind_cnt != -1) return nullptr;
                break;
            default:
                if (std::abs(edge_c->wind_cnt) != 1) return nullptr;
                break;
        }
        ...
        else if (pt == edge_o->local_min->vertex->pt && !IsOpenEnd(*edge_o->local_min->vertex))
        {
    ...
    else
    {
        ...
        else if (old_e1_windcnt == 1 && old_e2_windcnt == 1)
        {
            ...
            {
                case ClipType::Union:
                    if (e1Wc2 <= 0 && e2Wc2 <= 0) resultOp = AddLocalMinPoly(e1, e2, pt, false);
                    break;
                case ClipType::Difference:
...
{
    auto j = std::count_if(horz_seg_list_.begin(), horz_seg_list_.end(), [](HorzSegment& hs) { return UpdateHorzSegment(hs); });
    if (j < 2) return;
    ...
    for (; hs1 != hs_end1; ++hs1)
    {
        for (hs2 = hs1 + 1; hs2 != hs_end; ++hs2)
        {
            ...
            if (hs2->left_to_right == hs1->left_to_right || (hs2->right_op->pt.x <= hs1->left_op->pt.x)) continue;
            int64_t curr_y = hs1->left_op->pt.y;
            if (hs1->left_to_right)
            {
                while (hs1->left_op->next->pt.y == curr_y && hs1->left_op->next->pt.x <= hs2->left_op->pt.x)
                    hs1->left_op = hs1->left_op->next;
                while (hs2->left_op->prev->pt.y == curr_y && hs2->left_op->prev->pt.x <= hs1->left_op->pt.x)
                    hs2->left_op = hs2->left_op->prev;
                HorzJoin join = HorzJoin(DuplicateOp(hs1->left_op, true), DuplicateOp(hs2->left_op, false));
                horz_join_list_.push_back(join);
            }
            else
            {
                while (hs1->left_op->prev->pt.y == curr_y && hs1->left_op->prev->pt.x <= hs2->left_op->pt.x)
                    hs1->left_op = hs1->left_op->prev;
                while (hs2->left_op->next->pt.y == curr_y && hs2->left_op->next->pt.x <= hs1->left_op->pt.x)
                    hs2->left_op = hs2->left_op->next;
                HorzJoin join = HorzJoin(DuplicateOp(hs2->left_op, true), DuplicateOp(hs1->left_op, false));
                horz_join_list_.push_back(join);
            }
        }
    }
...
{
    Point64 ip;
    if (!GetIntersectPoint(e1.bot, e1.top, e2.bot, e2.top, ip)) ip = Point64(e1.curr_x, top_y);  // parallel edges
...
bool ClipperBase::ResetHorzDirection(const Active& horz, const Vertex* max_vertex, int64_t& horz_left, int64_t& horz_right)
{
...
inline bool HorzIsSpike(const Active& horzEdge)
{
    ...
    return (nextPt.y == horzEdge.bot.y) && (horzEdge.bot.x < horzEdge.top.x) != (horzEdge.top.x < nextPt.x);
}
 ...
 *******************************************************************************/
{
    ...
    if (vertex_max && !horzIsOpen && vertex_max != horz.vertex_top) TrimHorz(horz, PreserveCollinear);
    ...
    while (true)  // loop through consec. horizontal edges
    {
        ...
        while (e)
        {
            ...
            {
                if (IsHotEdge(horz) && IsJoined(*e)) Split(*e, e->top);
            ...
            if (vertex_max != horz.vertex_top || IsOpenEnd(horz))
            {
                ...
                if ((is_left_to_right && e->curr_x > horz_right) || (!is_left_to_right && e->curr_x < horz_left)) break;
        ...
        if (PreserveCollinear && !horzIsOpen && HorzIsSpike(horz)) TrimHorz(horz, true);
    ...
    while (e)
    {
        ...
        if (e->top.y == y)
        {
            ...
            else
            {
                ...
                UpdateEdgeIntoAEL(e);
                if (IsHorizontal(*e)) PushHorz(*e);  // horizontals are processed later
            }
        }
...
Active* ClipperBase::DoMaxima(Active& e)
{
    ...
    if (IsOpen(e))
    {
        if (IsHotEdge(e)) AddLocalMaxPoly(e, *max_pair, e.top);
...
{
    Active* prev = e.prev_in_ael;
    if (IsOpen(e) || !IsHotEdge(e) || !prev || IsOpen(*prev) || !IsHotEdge(*prev) || pt.y < e.top.y + 2 ||
        pt.y < prev->top.y + 2)  // avoid trivial joins
        return;
...
{
    Active* next = e.next_in_ael;
    if (IsOpen(e) || !IsHotEdge(e) || !next || IsOpen(*next) || !IsHotEdge(*next) || pt.y < e.top.y + 2 ||
        pt.y < next->top.y + 2)  // avoids trivial joins
        return;
...
inline bool GetHorzExtendedHorzSeg(OutPt*& op, OutPt*& op2)
{
    ...
    {
        while (op->prev != outrec->pts && op->prev->pt.y == op->pt.y) op = op->prev;
        while (op2 != outrec->pts && op2->next->pt.y == op2->pt.y) op2 = op2->next;
        return op2 != op;
    }
    else
    {
...
{
    if (!op || op->next == op || (!isOpen && op->next == op->prev)) return false;
...
bool ClipperBase::CheckBounds(OutRec* outrec)
{
    ...
    if (!outrec->pts || !BuildPath64(outrec->pts, ReverseSolution, false, outrec->path)) return false;
...
void ClipperBase::RecursiveCheckOwners(OutRec* outrec, PolyPath* polypath)
{
    ...
    while (outrec->owner && (!outrec->owner->pts || !CheckBounds(outrec->owner))) outrec->owner = outrec->owner->owner;

    if (outrec->owner && !outrec->owner->polypath) RecursiveCheckOwners(outrec->owner, polypath);
...
void ClipperBase::DeepCheckOwners(OutRec* outrec, PolyPath* polypath)
{
    ...
    while (outrec->owner && outrec->owner->splits)
    {
        ...
        for (auto s : *outrec->owner->splits)
        {
            ...
            if (split && split != outrec && split != outrec->owner && CheckBounds(split) &&
                split->bounds.Contains(outrec->bounds) && Path1InsidePath2(outrec->pts, split->pts))
            {
...
void Clipper64::BuildPaths64(Paths64& solutionClosed, Paths64* solutionOpen)
{
    ...
    for (size_t i = 0; i < outrec_list_.size(); ++i)
    {
        ...
        {
            if (BuildPath64(outrec->pts, ReverseSolution, true, path)) solutionOpen->emplace_back(std::move(path));
        }
        else
        {
            ...
            if (BuildPath64(outrec->pts, ReverseSolution, false, path)) solutionClosed.emplace_back(std::move(path));
        }
    }
}
...
void Clipper64::BuildTree64(PolyPath64& polytree, Paths64& open_paths)
{
    polytree.Clear();
    open_paths.resize(0);
    if (has_open_paths_) open_paths.reserve(outrec_list_.size());
    ...
    for (size_t i = 0; i < outrec_list_.size(); ++i)
    {
        ...
        {
            Path64 path;
            if (BuildPath64(outrec->pts, ReverseSolution, true, path)) open_paths.push_back(path);
            continue;
        }

        if (CheckBounds(outrec)) DeepCheckOwners(outrec, &polytree);
    }
}
...
void ClipperD::BuildPathsD(PathsD& solutionClosed, PathsD* solutionOpen)
{
    ...
    for (std::size_t i = 0; i < outrec_list_.size(); ++i)
    {
        ...
        if (solutionOpen && outrec->is_open)
        {
            if (BuildPathD(outrec->pts, ReverseSolution, true, path, invScale_)) solutionOpen->emplace_back(std::move(path));
        }
        else
        {
            CleanCollinear(outrec);
            // closed paths should always return a Positive orientation
            if (BuildPathD(outrec->pts, ReverseSolution, false, path, invScale_)) solutionClosed.emplace_back(std::move(path));
        }
    }
}
...
void ClipperD::BuildTreeD(PolyPathD& polytree, PathsD& open_paths)
{
    ...
    for (OutRec* outrec : outrec_list_)
    {
        ...
        {
            PathD path;
            if (BuildPathD(outrec->pts, ReverseSolution, true, path, invScale_)) open_paths.push_back(path);
            continue;
        }
