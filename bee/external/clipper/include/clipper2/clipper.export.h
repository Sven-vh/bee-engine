...
EXTERN_DLL_EXPORT CPaths64 ExecuteRectClip64(const CRect64& rect, const CPaths64 paths, bool convex_only = false);
EXTERN_DLL_EXPORT CPathsD ExecuteRectClipD(const CRectD& rect,
                                           const CPathsD paths,
                                           int precision = 2,
                                           bool convex_only = false);
EXTERN_DLL_EXPORT CPaths64 ExecuteRectClipLines64(const CRect64& rect, const CPaths64 paths);
EXTERN_DLL_EXPORT CPathsD ExecuteRectClipLinesD(const CRectD& rect, const CPathsD paths, int precision = 2);
                                 ...
                                 bool reverse_solution)
{
    ...
    if (sol_open.size() > 0) solution_open = CreateCPathsD(sol_open, 1 / scale);
    return 0;
}
...
EXTERN_DLL_EXPORT CPaths64 ExecuteRectClip64(const CRect64& rect, const CPaths64 paths, bool convex_only)
{
...
EXTERN_DLL_EXPORT CPathsD ExecuteRectClipD(const CRectD& rect, const CPathsD paths, int precision, bool convex_only)
{
    if (CRectIsEmpty(rect) || !paths) return nullptr;
...
EXTERN_DLL_EXPORT CPathsD ExecuteRectClipLinesD(const CRectD& rect, const CPathsD paths, int precision)
{
    if (CRectIsEmpty(rect) || !paths) return nullptr;
    if (precision < -8 || precision > 8) return nullptr;
    const double scale = std::pow(10, precision);
...
inline Paths64 ConvertCPaths64(const CPaths64& pp)
{
    ...
    if (pp)
    {
        ...
        ++v;                                              // skip cnts
        result.reserve(cnt);
        for (size_t i = 0; i < cnt; ++i) result.push_back(ConvertCPath64(*v++));
    }
    return result;
}
...
inline PathsD ConvertCPathsD(const CPathsD& pp)
{
    ...
    if (pp)
    {
        ...
        ++v;  // skip cnts path
        result.reserve(cnt);
        for (size_t i = 0; i < cnt; ++i) result.push_back(ConvertCPathD(*v++));
    }
    return result;
}
...
inline Paths64 ConvertCPathsD(const CPathsD& pp, double scale)
{
    ...
    if (pp)
    {
        ...
        result.reserve(cnt);
        ++v;  // skip cnts path
        for (size_t i = 0; i < cnt; ++i) result.push_back(ConvertCPathD(*v++, scale));
    }
    return result;
}
...
inline void InitCPolyPath64(CPolyTree64* cpt, bool is_hole, const std::unique_ptr<PolyPath64>& pp)
{
    ...
    for (const std::unique_ptr<PolyPath64>& pp_child : *pp) InitCPolyPath64(child++, !is_hole, pp_child);
}
...
inline CPolyTree64* CreateCPolyTree64(const PolyTree64& pt)
{
    ...
    for (const std::unique_ptr<PolyPath64>& pp : pt) InitCPolyPath64(child++, true, pp);
    return result;
}
...
inline void DisposeCPolyPath64(CPolyPath64* cpp)
{
    ...
    for (size_t i = 0; i < cpp->child_count; ++i) DisposeCPolyPath64(child);
    delete[] cpp->childs;
}
...
inline void InitCPolyPathD(CPolyTreeD* cpt, bool is_hole, const std::unique_ptr<PolyPath64>& pp, double scale)
{
    ...
    for (const std::unique_ptr<PolyPath64>& pp_child : *pp) InitCPolyPathD(child++, !is_hole, pp_child, scale);
}
...
inline CPolyTreeD* CreateCPolyTreeD(const PolyTree64& pt, double scale)
{
    ...
    for (const std::unique_ptr<PolyPath64>& pp : pt) InitCPolyPathD(child++, true, pp, scale);
    return result;
}
...
inline void DisposeCPolyPathD(CPolyPathD* cpp)
{
    ...
    for (size_t i = 0; i < cpp->child_count; ++i) DisposeCPolyPathD(child++);
    delete[] cpp->childs;
}
