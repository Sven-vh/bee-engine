...
inline void BooleanOp(ClipType cliptype, FillRule fillrule, const Paths64& subjects, const Paths64& clips, PolyTree64& solution)
{
    Paths64 sol_open;
    Clipper64 clipper;
    clipper.AddSubject(subjects);
...
inline PathsD BooleanOp(ClipType cliptype, FillRule fillrule, const PathsD& subjects, const PathsD& clips, int precision = 2)
{
    int error_code = 0;
...
inline Paths64 ExecuteRectClip(const Rect64& rect, const Paths64& paths, bool convex_only)
{
...
inline Paths64 ExecuteRectClip(const Rect64& rect, const Path64& path, bool convex_only)
{
...
inline PathsD ExecuteRectClip(const RectD& rect, const PathsD& paths, bool convex_only, int precision = 2)
{
    ...
    return ScalePaths<double, int64_t>(rc.Execute(pp, convex_only), 1 / scale, error_code);
}

inline PathsD ExecuteRectClip(const RectD& rect, const PathD& path, bool convex_only, int precision = 2)
{
...
static void OutlinePolyPath(std::ostream& os, bool isHole, size_t count, const std::string& preamble)
{
    std::string plural = (count == 1) ? "." : "s.";
    if (isHole)
    {
        if (count)
            os << preamble << "+- Hole with " << count << " nested polygon" << plural << std::endl;
        else
    ...
    {
        if (count)
            os << preamble << "+- Polygon with " << count << " hole" << plural << std::endl;
        else
            os << preamble << "+- Polygon" << std::endl;
    }
}

static void OutlinePolyPath64(std::ostream& os, const PolyPath64& pp, std::string preamble, bool last_child)
{
    ...
    if (pp.Count())
    {
        ...
        for (; it < pp.end() - 1; ++it) OutlinePolyPath64(os, **it, preamble, false);
        OutlinePolyPath64(os, **it, preamble, true);
    }
}

static void OutlinePolyPathD(std::ostream& os, const PolyPathD& pp, std::string preamble, bool last_child)
{
    OutlinePolyPath(os, pp.IsHole(), pp.Count(), preamble);
    preamble += (!last_child) ? "|  " : "   ";
    if (pp.Count())
    {
        ...
        for (; it < pp.end() - 1; ++it) OutlinePolyPathD(os, **it, preamble, false);
...
{
    PolyPath64List::const_iterator it = pp.begin();
    for (; it < pp.end() - 1; ++it) details::OutlinePolyPath64(os, **it, "   ", false);
...
{
    PolyPathDList::const_iterator it = pp.begin();
    for (; it < pp.end() - 1; ++it) details::OutlinePolyPathD(os, **it, "   ", false);
...
inline constexpr void MakePathGeneric(const T list, size_t size, std::vector<U>& result)
{
...
template <typename T, typename std::enable_if<std::is_arithmetic<T>::value && !std::is_same<char, T>::value, bool>::type = true>
...
inline Path64 TrimCollinear(const Path64& p, bool is_open_path = false)
{
    ...
    if (!is_open_path)
    {
        while (srcIt != stop && !CrossProduct(*stop, *srcIt, *(srcIt + 1))) ++srcIt;
        while (srcIt != stop && !CrossProduct(*(stop - 1), *stop, *srcIt)) --stop;
    ...
    {
        while (dst.size() > 2 && !CrossProduct(dst[dst.size() - 1], dst[dst.size() - 2], dst[0])) dst.pop_back();
...
{
    return Ellipse(rect.MidPoint(), static_cast<double>(rect.Width()) * 0.5, static_cast<double>(rect.Height()) * 0.5, steps);
}
...
inline Path<T> Ellipse(const Point<T>& center, double radiusX, double radiusY = 0, int steps = 0)
{
    ...
    if (steps <= 2) steps = static_cast<int>(PI * sqrt((radiusX + radiusY) / 2));
...
inline Path<T> SimplifyPath(const Path<T> path, double epsilon, bool isClosedPath = true)
{
    ...
    for (;;)
    {
        ...
        if (distSqr[next] < distSqr[curr])
        {
            ...
            if (next != high || isClosedPath) distSqr[next] = PerpendicDistFromLineSqrd(path[next], path[curr], path[next2]);
            curr = next;
        }
        else
        {
            ...
            distSqr[curr] = PerpendicDistFromLineSqrd(path[curr], path[prior], path[next]);
            if (prior != 0 || isClosedPath) distSqr[prior] = PerpendicDistFromLineSqrd(path[prior], path[prior2], path[curr]);
        }
    }
    Path<T> result;
    result.reserve(len);
...
inline Paths<T> SimplifyPaths(const Paths<T> paths, double epsilon, bool isClosedPath = true)
{
    Paths<T> result;
    result.reserve(paths.size());
    for (const auto& path : paths) result.push_back(SimplifyPath(path, epsilon, isClosedPath));
    return result;
}
