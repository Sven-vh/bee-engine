...
struct Rect
{
    ...
    bool Contains(const Point<T>& pt) const { return pt.x > left && pt.x < right && pt.y > top && pt.y < bottom; }
...
static const Rect64 MaxInvalidRect64 = Rect64(INT64_MAX, INT64_MAX, INT64_MIN, INT64_MIN);
static const RectD MaxInvalidRectD = RectD(MAX_DBL, MAX_DBL, -MAX_DBL, -MAX_DBL);
...
inline Path<T1> ScalePath(const Path<T2>& path, double scale_x, double scale_y, int& error_code)
{
    ...
    std::transform(path.begin(),
                   ...
                   [scale_x, scale_y](const auto& pt) { return Point<T1>(pt.x * scale_x, pt.y * scale_y, pt.z); });
#else
    std::transform(path.begin(),
                   path.end(),
                   back_inserter(result),
                   [scale_x, scale_y](const auto& pt) { return Point<T1>(pt.x * scale_x, pt.y * scale_y); });
#endif
    return result;
}
...
template <typename T1, typename T2>
inline Paths<T1> ScalePaths(const Paths<T2>& paths, double scale_x, double scale_y, int& error_code)
{
    Paths<T1> result;
    ...
    if constexpr (std::numeric_limits<T1>::is_integer && !std::numeric_limits<T2>::is_integer)
    {
        ...
        if ((r.left * scale_x) < min_coord || (r.right * scale_x) > max_coord || (r.top * scale_y) < min_coord ||
    ...
    std::transform(paths.begin(),
                   paths.end(),
                   back_inserter(result),
                   [=, &error_code](const auto& path) { return ScalePath<T1, T2>(path, scale_x, scale_y, error_code); });
    return result;
}
...
inline Path<T1> TransformPath(const Path<T2>& path)
{
    Path<T1> result;
    result.reserve(path.size());
    std::transform(path.cbegin(), path.cend(), std::back_inserter(result), [](const Point<T2>& pt) { return Point<T1>(pt); });
    return result;
}
...
inline PathD Path64ToPathD(const Path64& path) { return TransformPath<double, int64_t>(path); }

inline PathsD Paths64ToPathsD(const Paths64& paths) { return TransformPaths<double, int64_t>(paths); }

inline Path64 PathDToPath64(const PathD& path) { return TransformPath<int64_t, double>(path); }

inline Paths64 PathsDToPaths64(const PathsD& paths) { return TransformPaths<int64_t, double>(paths); }
...
inline Path<T> StripNearEqual(const Path<T>& path, double max_dist_sqrd, bool is_closed_path)
{
    ...
    while (result.size() > 1 && NearEqual(result.back(), first_pt, max_dist_sqrd)) result.pop_back();
    return result;
}
...
inline Paths<T> StripNearEqual(const Paths<T>& paths, double max_dist_sqrd, bool is_closed_path)
{
    Paths<T> result;
    result.reserve(paths.size());
    for (typename Paths<T>::const_iterator paths_citer = paths.cbegin(); paths_citer != paths.cend(); ++paths_citer)
    {
...
{
    for (typename Paths<T>::iterator paths_citer = paths.begin(); paths_citer != paths.end(); ++paths_citer)
    {
...
inline double Area(const Path<T>& path)
{
    ...
    if (cnt & 1) a += static_cast<double>(it2->y + it1->y) * (it2->x - it1->x);
    return a * 0.5;
}
...
{
    double a = 0.0;
    for (typename Paths<T>::const_iterator paths_iter = paths.cbegin(); paths_iter != paths.cend(); ++paths_iter)
    {
        a += Area<T>(*paths_iter);
    }
    return a;
...
inline bool GetIntersectPoint(const Point64& ln1a, const Point64& ln1b, const Point64& ln2a, const Point64& ln2b, Point64& ip)
{
...
inline Point64 GetClosestPointOnSegment(const Point64& offPt, const Point64& seg1, const Point64& seg2)
{
