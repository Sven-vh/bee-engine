...
struct IntersectNode
{
    ...
    Active* edge1;
    Active* edge2;
    IntersectNode() : pt(Point64(0, 0)), edge1(nullptr), edge2(nullptr) {}
...
#ifdef USINGZ
typedef std::function<void(const Point64& e1bot, const Point64& e1top, const Point64& e2bot, const Point64& e2top, Point64& pt)>
    ZCallback64;

typedef std::function<void(const PointD& e1bot, const PointD& e1top, const PointD& e2bot, const PointD& e2top, PointD& pt)>
    ZCallbackD;
#endif
...
public:
    ...
    const PolyPath64* operator[](size_t index) const { return childs_[index].get(); }

    const PolyPath64* Child(size_t index) const { return childs_[index].get(); }
...
public:
    explicit PolyPathD(PolyPathD* parent = nullptr) : PolyPath(parent) { inv_scale_ = parent ? parent->inv_scale_ : 1.0; }

    ~PolyPathD() { childs_.resize(0); }

    const PolyPathD* operator[](size_t index) const { return childs_[index].get(); }

    const PolyPathD* Child(size_t index) const { return childs_[index].get(); }
...
public:
    ...
    void AddOpenSubject(const Paths64& open_subjects) { AddPaths(open_subjects, PathType::Subject, true); }
    ...
    bool Execute(ClipType clip_type, FillRule fill_rule, Paths64& closed_paths, Paths64& open_paths)
    {
        closed_paths.clear();
        open_paths.clear();
        if (ExecuteInternal(clip_type, fill_rule, false)) BuildPaths64(closed_paths, &open_paths);
        CleanUp();
        return succeeded_;
    }
    ...
    bool Execute(ClipType clip_type, FillRule fill_rule, PolyTree64& polytree, Paths64& open_paths)
    {
...
public:
    ...
    void ZCB(const Point64& e1bot, const Point64& e1top, const Point64& e2bot, const Point64& e2top, Point64& pt)
    {
        // de-scale (x & y)
    ...
    bool Execute(ClipType clip_type, FillRule fill_rule, PolyTreeD& polytree, PathsD& open_paths)
    {
#ifdef USINGZ
        CheckCallback();
#endif
