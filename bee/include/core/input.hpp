...
public:
    ...
    virtual float GetGamepadAxisPrevious(int gamepadID, GamepadAxis axis) const = 0;

    /// Checks and returns whether a given gamepad button is being held down in the current frame.
    virtual bool GetGamepadButton(int gamepadID, GamepadButton button) const = 0;

    /// Checks and returns whether a given gamepad button is being pressed in the current frame without having been pressed in
    /// the previous frame.
    virtual bool GetGamepadButtonOnce(int gamepadID, GamepadButton button) const = 0;
    ...
    /// previous frame.
    virtual bool GetMouseButtonOnce(MouseButton button) const = 0;

    /// Gets the screen position of the mouse, relative to the top-left corner of the screen.
    virtual glm::vec2 GetMousePosition() const = 0;

    /// Gets the screen position of the mouse in the previous frame, relative to the top-left corner of the screen.
    virtual glm::vec2 GetPreviousMousePosition() const = 0;
    ...
    virtual bool GetKeyboardKey(KeyboardKey button) const = 0;

    /// Checks and returns whether a given keyboard key is being pressed in the current frame without having been pressed in the
    /// previous frame.
    virtual bool GetKeyboardKeyOnce(KeyboardKey button) const = 0;

    static Input* Create();

private:
    friend class EngineClass;
    friend class InputPc;
    friend class InputProspero;
